...
const CancelReceiptReport = () => {
  ...
  const fetchReport = async () => {
    ...
    try {
      setLoading(true);
      const token = localStorage.getItem('token');
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/reports/cancel-receipt?companyId=${selectedCompany}&fromDate=${fromDate}&toDate=${toDate}`, {
        headers: { Authorization: `Bearer ${token}` }
      });
      setReport(res.data.data);
    } catch (err) {
      console.error('Error fetching cancel receipt report', err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <main className="main-content">
      <header className="dashboard-header">
        <div className="welcome-section">
          <h1>Cancel Receipt Report</h1>
          <p>Detailed log of all voided or cancelled receipt vouchers.</p>
        </div>
      </header>
      ...
      <section className="content-section content-section-elite">
        ...
        {loading ? (
                ...
                <tbody>
                  {report.length > 0 ? report.map((r, idx) => (
                    <tr key={idx} className="hoverable">
                      ...
                      <td className="font-bold">{r.receiptNumber}</td>
                      <td>
                        <div className="particulars-info">
                          <span className="main-info">{r.payer?.clientName || r.payer?.companyName || 'N/A'}</span>
                      ...
                      <td>
                        <div className="particulars-info">
                          <span className="main-info">{r.receivedBy?.username}</span>
                        </div>
                      </td>
                    </tr>
                  )) : (
                    <tr>
                      <td colSpan="7">
                        <div className="empty-state-elite">
                          ...
                          <p>No cancelled receipts found for the selected period.</p>
                        </div>
                      </td>
                    </tr>
                  )}
          ...
          <div className="empty-state-elite">
            ...
            <p>Select a company and date range to view cancelled receipts.</p>
          </div>
        )}
      </section>
    </main>
...
export default CancelReceiptReport;
